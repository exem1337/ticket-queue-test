<template>
  <form class="editor-form shadow" @submit.prevent="editType">
    ...
    <input
      ...
      class="select"
      v-model="editTicketType"
    />
    <button
      type="submit"
      class="button-pushable queue"
      :disabled="editTypeInputNotEmpty"
      role="button"
    >
      <span class="button-shadow"></span>
      <span class="button-edge"></span>
      <span class="button-front text"> Редактировать тип </span>
    </button>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const editTypeInputNotEmpty = computed(() => {
      return !editTicketType.value.length
    });
    ...
    const editType = () => {
      store.commit("EDIT_TYPES", {
        key: props.type.key,
        typeName: editTicketType.value,
      } as ticketType);
