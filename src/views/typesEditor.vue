    ...
    <div class="container">
        ...
        <div class="speed-editor shadow">
            ...
            <div class="speed-items">
                <div class="speed-item" title="30 секнуд на первого, 5 минут на всех остальных" @click="changeSpeed(300000)" :class="{active: store.state.queueSpeed === 300000}">Обычная</div>
                <div class="speed-item" title="15 секнуд на первого, 2.5 минут на всех остальных" @click="changeSpeed(150000)" :class="{active: store.state.queueSpeed === 150000}">Быстрая</div>
                <div class="speed-item" title="меньше секнуды на первого, 7.5 секунд на всех остальных" @click="changeSpeed(7500)" :class="{active: store.state.queueSpeed === 7500}">Скорость света</div>
            </div>
        </div>
...
export default defineComponent ({
    ...
    setup() {
        ...
        const changeSpeed = function(speed :number) { //приходится чистить очередь из-за модалок
        ...
        const createType = () => {
            store.commit('CREATE_TICKET_TYPE', { typeName : newTicketType.value.trim(), key : +Date.now() } as ticketType)
            updateTypesLocalStorage()
            newTicketType.value = ''
        }

        const newTypeInputNotEmpty = computed(() => {
            return newTicketType.value.length !== 0 ? false : true
        })
